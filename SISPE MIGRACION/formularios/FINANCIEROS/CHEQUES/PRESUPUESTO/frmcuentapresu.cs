...
{
    public partial class frmcuentapresu : Form
    {
        ...
        public frmcuentapresu()
        {
            InitializeComponent();
            this.KeyPreview = true;

            this.KeyPress += new KeyPressEventHandler(frmcuentapresu_KeyPress);
        ...
        {
            string query = "SELECT * FROM financieros.cuentas_presupuesto order by id";
        ...
        private void data_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Delete)
            {
                ...
                {
                    string query = "delete from financieros.cuentas_presupuesto where id='{0}'";
            ...
            if (e.KeyCode == Keys.F9)
            {
                ...
                if (dialogo == DialogResult.Yes)
                {
                    string query = "update financieros.cuentas_presupuesto set cuenta='{0}',descripcion='{1}' where id='{2}'";
        ...
        private void txtdesc_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (dialogo == DialogResult.Yes)
                {
                    ...
                    string maxid = "select (max(id)+1) as  id from financieros.cuentas_presupuesto";
                    List<Dictionary<string, object>> lis = globales.consulta(maxid);
                    string maximid = Convert.ToString(lis[0]["id"]);
                    string qry = "insert into financieros.cuentas_presupuesto (cuenta,descripcion,id) values ('{0}','{1}','{2}')";
        ...
        private void frmcuentapresu_KeyPress(object sender, KeyPressEventArgs e)
        {
            char S;
        ...
        private void textBusqueda_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            string query = string.Format("select * from financieros.cuentas_presupuesto where cuenta like '{0}%'  limit 30;", busqueda);
