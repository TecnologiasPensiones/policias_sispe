...
{
    public partial class frmTasaHipote : Form
    {
        private List<string> relacionesLaborales;
        public frmTasaHipote()
        {
            InitializeComponent();
        }

        private void frmTasaHipote_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                txtTasas_q.Text = Convert.ToString(resultado[0]["tasa_h"]);
        ...
        private void datos_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                string query = $"update  catalogos.tasa set tasa = {tasaAnual} where fmodif = '{fecha}' and trel = '{tipoRelacion}' and t_prestamo = 'H'";
        ...
        private void txtTasas_q_TextChanged(object sender, EventArgs e)
        {
           
        }

        private void cmbDescripcion_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (tasa.aceptar)
            {
                ...
                if (resultado == DialogResult.Yes)
                {
                    ...
                    {
                        query = string.Format("insert into catalogos.tasa(trel,fmodif,tasa,uuqm,fum,hum,t_prestamo) values ('{0}','{1}',{2},'{3}','{4}','{5}','{6}')", item, txt1Fecha, txt1Interes, "T", fechaActual, horaActual, "H");
                        if (!globales.consulta(query, true))
                ...
                {
                    query = string.Format("insert into catalogos.tasa(trel,fmodif,tasa,uuqm,fum,hum,t_prestamo) values ('{0}','{1}',{2},'{3}','{4}','{5}','{6}')", txtTrel.Text, txt1Fecha, txt1Interes, "T", fechaActual, horaActual, "H");
        ...
        private void cmbDescripcion_TextChanged(object sender, EventArgs e)
        {
            ...
            txtTasas_q.Text = Convert.ToString(resultado[0]["tasa_h"]);
            datos.Rows.Clear();
            query = string.Format("select * from catalogos.tasa where trel = '{0}' and t_prestamo = 'H' order by id desc", txtTrel.Text);
        ...
        {
            globales.showModal(new frmImprimirReporteTasas(true));
        }
    }
}
