...
{
    public partial class frmaclaraciones : Form
    {
        ...
        private void txtBusqueda_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (txtBusqueda.Text.Contains("..") || txtBusqueda.Text.Contains("."))
            {
                ...
                foreach (string i in split)
                {
                    ...
                    nombre_em += $" nombre_em ilike '%{i}%' ,";
                }
                nombre_em = nombre_em.Substring(0, nombre_em.Length - 1).Replace(",", " and ");
                query = $"SELECT ID,rfc,nombre_em FROM  datos.empleados WHERE pendiente = {!chkFusion.Checked} AND({nombre_em}) ORDER BY nombre_em  limit 50";
            }
            else {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (!chkFusion.Checked)
            {
                frmfechareg c_fecha = new frmfechareg(this);
                c_fecha.ShowDialog();
                fecha_reg = string.Format("{0:yyyy-MM-dd}", fec1);
                bandera = true;
            }
            foreach (DataGridViewRow row in datos.Rows)
            {
                ...
                rfcnuevo = Convert.ToString(row.Cells[3].Value);
                ...
                if (movimiento == "F" && chkFusion.Checked)
                {
                    ...
                    {
                        globales.MessageBoxError("ERROR, FAVOR DE VERIFICAR INFORMACIÓN","Aviso",globales.menuPrincipal);
                        datos.Rows.Clear();

                    }
                }
            }
            globales.MessageBoxSuccess("Proceso afectuado correctamente.","Aviso",globales.menuPrincipal);
            datos.Rows.Clear();
            txtBusqueda.Clear();
        }
